package uk.jamesdal.perfmock.test.unit.lib.script;
...
import uk.jamesdal.perfmock.lib.script.ScriptedAction;
...
public class ScriptedActionTests extends TestCase {
    ...
    public void testInterpretsCallbackExpression() throws Exception {
        context.checking(new Expectations() {{
            oneOf (callout).doSomethingWith(callback); will(ScriptedAction.perform("$0.callback()"));
    ...
    public void testScriptCanReferToParametersByIndex() {
        context.checking(new Expectations() {{
            oneOf (callout).doSomethingWithBoth(callback, callback2); will(ScriptedAction.perform("$1.callback()"));
    ...
    public void testScriptCanReferToInvokedObjectAs$This() throws Exception {
        context.checking(new Expectations() {{
            oneOf (callout).doSomethingWith(callback); will(ScriptedAction.perform("$0.callbackWith($this)"));
    ...
    public void testCanBindValuesFromTestIntoScript() throws Exception {
        ...
        context.checking(new Expectations() {{
            oneOf (callout).doSomethingWith(callback); will(ScriptedAction.perform("$0.callbackWith(x, y)").where("x", v1).where("y", v2));
    ...
    public void testExceptionThrownByCallbackIsPassedBackToCaller() throws Exception {
        ...
        context.checking(new Expectations() {{
            oneOf (callout).doSomethingWith(callback); will(ScriptedAction.perform("$0.throwException()"));
