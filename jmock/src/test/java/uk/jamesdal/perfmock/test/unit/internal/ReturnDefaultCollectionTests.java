...
import uk.jamesdal.perfmock.internal.ReturnDefaultValueAction;
...
public class ReturnDefaultCollectionTests {
    ...
    imposterisesBeanContextTypesToAvoidMissingTypesInAndroid() throws Throwable {
        assertThat(action.invoke(ReturnDefaultValueActionTests.invocationReturning(BeanContext.class)).getClass(), IS_PROXY_CLASS);
        assertThat(action.invoke(ReturnDefaultValueActionTests.invocationReturning(BeanContextServices.class)).getClass(), IS_PROXY_CLASS);
    }
    ...
    imposterisesUnsupportedMapTypes() throws Throwable {
        assertThat(action.invoke(ReturnDefaultValueActionTests.invocationReturning(LogicalMessageContext.class)).getClass(), IS_PROXY_CLASS);
    }
    
    @Test public void
    returnsNullForAbstractCollections() throws Throwable {
        assertThat(action.invoke(ReturnDefaultValueActionTests.invocationReturning(AbstractCollection.class)), is(nullValue()));
    }

    private void returnsInstanceForType(Class<?> declaredType, Class<?> expectedType) throws Throwable {
        assertThat(
                action.invoke(ReturnDefaultValueActionTests.invocationReturning(declaredType)),
                instanceOf(expectedType));
    }

    @SuppressWarnings("unchecked")
    private ArrayList<Object> returnedArrayList() throws Throwable {
        return (ArrayList<Object>) action.invoke(ReturnDefaultValueActionTests.invocationReturning(ArrayList.class));
    }
